...
class PontosProfileSelect(SelectEntity):
    ...
    def _profile_name_changed(self, event: Event):
        ...
        # <-- NEW: Also re-check the current numeric code
        #     in case it was None before and is now valid
        sensor_entity_id = er.async_get(self._hass).async_get_entity_id(
            "sensor", DOMAIN, self._sensor_unique_id
        )
        if sensor_entity_id:
            ...
            if st:
                code_str = st.state
                # If the name wasn't available before, maybe it is now
                new_name = self._code_to_name(code_str)
                if new_name != self._attr_current_option:
                    LOGGER.debug(f"After name sensors updated, re-check code {code_str} => {new_name}")
                    self.set_state(new_name)
